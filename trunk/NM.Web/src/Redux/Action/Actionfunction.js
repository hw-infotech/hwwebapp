...
import {
  API_URL,
  CONTACT_US_REGISTER_USER,
  EXAMPLE,
  MSQL_API_URL,
  REGISTER_APP_DESIGN_USER,
  REGISTER_GRAPHIC_DESIGN_USER,
  REGISTER_SEO_DESIGN_USER,
  REGISTER_UIUX_DESIGN_USER,
  REGISTER_WEB_DESIGN_USER,
...
export const graphicDesignuserRegister = (user) => (dispatch) => {
  apidata
    ...
    .catch((err) => {
      dispatch({
        type: REGISTER_GRAPHIC_DESIGN_USER,
...
export const UIUXuserRegister = (user) => (dispatch) => {
  apidata
    ...
    .catch((err) => {
      dispatch({
        type: REGISTER_UIUX_DESIGN_USER,
        payload: { data: false, err },
      });
      const notify = () => toast.error(`${err.response.data.error}`); //, { theme: "colored" }
      console.log("err ", err);
...
export const contactUSRegister = (user) => (dispatch) => {
  apidata
    .post(`Contact/create`, user)
    .then((res) => {
      dispatch({ type: CONTACT_US_REGISTER_USER, payload: res?.data });
      if (res.data.success) {
        const notify = () =>
          toast.success(
            "Thank you for contacting us,we will respond you as quickly as possible."
          ); //, { theme: "colored" }
        notify();
      } else {
...
export const subscribeNewsletter = (email) => (dispatch) => {
  ...
  apidata
    .post(`NewsLetter/Subscribe`, value, { headers })
    .then((response) => {
      dispatch({
...
export const createResume = (resume, history) => (dispatch) => {
  apidata
