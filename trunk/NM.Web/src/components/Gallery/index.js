        ...
        <Dashboard >
            <Modal show={show} onHide={handleClose}>
                <Modal.Header closeButton>
                    <div id="storyDiv" onLoad={() => {
                        let maindiv = document.querySelectorAll("#storyDiv")[0].children[0]
                    ...
                    <Slider {...settings} className="storySlider">
                        {stories12?.map((story, index) => {
                            return <div key={index} className="sliderFirstDiv">
                                <img src={story.posts[0].url} alt="slider image" height={160} onClick={() => {
                                    setCurrrentIndex(0)
                                    setAudio(story.audio)
                                    setIndexPost({ ...indexPost, posts: story.posts, id: index })
                                    setSelected(index)
                                }} />
                                <span className={selected == index ? "red" : "white"}>{story.event_title}</span>
                            </div>
                        })}
                    </Slider>
                </Modal.Header>
                ...
                <div className="galleryMain">
                    <div className="p-2">
                        <div>
                            <Row>
                                {stories12?.map((story, index) => (
                                    <Col key={index} sm={2} xs={2} xxl={3} lg={3} xl={3} md={3} className="p-1">
                                            ...
                                            <div className="innerCard">
                                                ...
                                                <div className="event_div">
                                                    <div className="event_content">
                                                        <label className="event_title">{story.event_title}</label>
                                                        <label className="event_date">Date {moment(`${story.createAt}`).format('DD/MM/yyyy, h:m:s A')}</label>
                                                    </div>
